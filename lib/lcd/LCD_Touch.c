...
static uint8_t TP_Scan(uint8_t chCoordType)
{
    ...
    if (!DEV_Digital_Read(TP_IRQ_PIN)) {//Press the button to press
        ...
        if (chCoordType) {
			...
			if(LCD_2_8==id){
                //This code was copied from the 3.5" code and modified slightly
    ...
    while (1) {
        ...
        if((sTP_DEV.chStatus & 0xC0) == TP_PRESSED) {
            ...
            case 4:
                ...
                if (LCD_2_8==id)
                {
                    // This code is a modified version of the 3.5" Code
...
{
	if(LCD_2_8==id){
		// Code below is a modified version of the 3.6" code 
        if(	sTP_DEV.TP_Scan_Dir == D2U_L2R ) { //SCAN_DIR_DFT = D2U_L2R
			sTP_DEV.fXfac = -0.091077;
			sTP_DEV.fYfac = 0.068571;
			sTP_DEV.iXoff = 355;
			sTP_DEV.iYoff = -18;
		} else if( sTP_DEV.TP_Scan_Dir == L2R_U2D ) {
			...
			sTP_DEV.fXfac = -0.068571;
			sTP_DEV.fYfac = -0.092703;
			sTP_DEV.iXoff = 258;
			sTP_DEV.iYoff = 359;
		} else if( sTP_DEV.TP_Scan_Dir == U2D_R2L ) {
			sTP_DEV.fXfac = 0.092529;
			sTP_DEV.fYfac = -0.069120;
			sTP_DEV.iXoff = -37;
			sTP_DEV.iYoff = 260;
		} else {
			LCD_Clear(LCD_BACKGROUND);
		...
		if(	sTP_DEV.TP_Scan_Dir == D2U_L2R ) { //SCAN_DIR_DFT = D2U_L2R
			...
			sTP_DEV.fXfac = -0.092298;
			sTP_DEV.fYfac = 0.068528;
			sTP_DEV.iXoff = 354;
			sTP_DEV.iYoff = -18;
		} else {
			LCD_Clear(LCD_BACKGROUND);
